...
public abstract class PageHandler<T> {
  ...
  public void addPages(NBTTagList pages, T obj, ItemStackCallback callback) {
    ...
    for (int i = 0; i < itemStacksLength(obj); i++) {
      ...
      if (displayStackSize() && itemStack.getCount() > 1) stringBuilder.append(itemStack.getCount()).append(TEXT_AFTER_STACK_SIZE);
  ...
  public static boolean empty(List<ItemStack> itemStacks) {
    return empty(itemStacks, 0, itemStacks.size() - 1);
  }

  public static boolean empty(List<ItemStack> itemStacks, int minIndex, int maxIndex) {
    for (int i = minIndex; i <= maxIndex; i++) {
      if (!itemStacks.get(i).isEmpty()) return false;
    }
    return true;
  }
  ...
  public static abstract class IInventoryHandler extends PageHandler<EntityPlayer> {
    ...
    public boolean valid(EntityPlayer entityPlayer) {
      ...
      for (int i = 0; i< iInventory.getSizeInventory(); i++) {
        if (!iInventory.getStackInSlot(i).isEmpty()) return true;
      }
      return false;
    }
