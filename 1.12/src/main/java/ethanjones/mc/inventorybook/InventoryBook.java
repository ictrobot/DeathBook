...
public class InventoryBook {
  ...
  public void rightClick(PlayerInteractEvent.RightClickItem event) {
    ...
    if (event.getItemStack().isEmpty()  || event.getItemStack().getItem() != Items.WRITABLE_BOOK || !event.getEntityPlayer().isSneaking()) return;
    if (event.isCanceled() || event.getEntityPlayer().world.isRemote || event.getEntityPlayer() instanceof FakePlayer) return;
    ItemStack book = getBook(event.getEntityPlayer(), "Inventory");
    if (book.isEmpty() || !decrementBook(event.getEntityPlayer())) return;
  ...
  public ItemStack getBook(EntityPlayer player, String title) {
    try {
      ...
      if (pages.hasNoTags()) return ItemStack.EMPTY;
